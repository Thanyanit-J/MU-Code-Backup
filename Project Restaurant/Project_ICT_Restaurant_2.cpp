...
int main(){
	...
	for(int num_table = 1; num_table < total_table; num_table++){
...
void go_menu(){
	...
	do{
	    ...
	    gets(choice_tmp);
	    isFirstInput = no;
	}while(isValidNumber(choice_tmp,0,3)==no);
    ...
    switch(choice){
    	...
    	case 0: isEnd = yes; /*END PROGRAM*/break;
	}
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void go_bookTable(){
    ...
    while(1){
		...
		if(book_table!=0){
			if(isOccupied[book_table-1])
				...
				return; //DONE BOOKING AND BACK TO MENU
			}
		}
		else{
			return; //BACK TO MENU
...
void go_orderFoodAndDrink(){
	...
	do{
		...
		printf("Enter the table number [1-%d] (0 to exit): ",total_table);
		gets(num_table_tmp);
		isFirstInput=no;
		
		if(isValidNumber(num_table_tmp,0,total_table)){
			num_table = atoi(num_table_tmp);
			if(num_table==0) return;
...
void go_displayAndClearBill(){
	...
	//if there's no order.
	if(isOrderFood[num_table-1]==0 && isOrderDrink[num_table-1]==0){//if no food or drink is ordered yet, return to menu
	...
	if(isOrderFood[num_table-1]==no) printf("None\n");
	printf("-----------------------------------------------------------------\n");
	printf("\n");
	...
	if(isOrderDrink[num_table-1]==no) printf("None\n");
	printf("-----------------------------------------------------------------\n");
	printf("\n");
...
int isValidChar(char input[10], char *ptr){ //check if input is either 'y' or 'n', also assign 'y' or 'n' via pointer
	...
	for(int i=0;i<strlen(input);i++){ //check if input if either 'y' or 'n'
