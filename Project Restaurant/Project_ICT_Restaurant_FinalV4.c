...
void go_orderFoodAndDrink(){
	...
	do{
		...
		else if(isNoOccupiedTable==yes && Takehome == no){ //if no table is occupied yet, return to menu
				printf("There is no one sitting at the table\n");
        		printf("Press Enter to go back to main menu.");  
        		getch();
        		go_menu();
...
void go_displayAndClearBill(){
	...
	for(int i=0;i<10;i++)
	{
		if(num_table == 11)
		{
			if(Takehome_food[1][i] == 0 && Takehome_drink[1][i]==0)
			{
    ...
    do{
		...
		if(isValidChar(confirm_tmp,&confirm)){
			if(confirm=='y'){
				...
				for(int i=0;i<total_table;i++)
				{
					if(isOccupied[i] == 0 && Takehome == no)
					{
						isNoOccupiedTable = yes;
					}
				}
			}
			go_menu();
